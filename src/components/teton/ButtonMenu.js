...
export default function ButtonMenu() {
  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button className="flex-shrink-0 p-2.5 m-1 text-gray-500 rounded-full hover:text-gray-900 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-opacity-60 bg-white  top-1.5 right-1.5 shadow border-gray-250 border">
          <DotsVerticalIcon
            className="w-6 h-6"
            aria-hidden="true"
          />
      ...
      >
        <Menu.Items className="absolute bg-white w-96 rounded-md shadow p-1 mt-1.5 right-0">
          {menu.map((item) => ( 
            <Menu.Item>
              <a
                href='/'
